...
CDASHPlayer.prototype.OnMessage = function( session, event )
{
	...
	xhr.open( 'POST', 'http://store.steampowered.com/video/license/' + this.m_strUniqueId, true );
...
CDASHPlayerUI.prototype.OnVideoInitialized = function()
{
	...
	if ( !this.m_player.BPlayingAudio() )
	{
		...
		$J( this.m_player.m_elVideoPlayer ).one( 'bufferingcomplete', function() { _ui.DisplayNotification( '此视频不包含任何音频', 15000 ) } );
	}
	else
	{
...
CDASHPlayerUI.prototype.InitSettingsPanelInUI = function()
{
	...
	if ( this.m_player.BIsLiveContent() )
	{
		...
		if ( rgRepresentation.length > 1 )
			this.m_elLiveResSelectorPanel.append( '<span data-value="-1">自动</span>' );
		...
		for ( var r = 0; r < rgRepresentation.length; r++ )
		{
			if ( rgRepresentation[ r ].height.toString().length > 1 )
			{
				...
				var strBandwidth = ( rgRepresentation[ r ].bandwidth / 1000000 ).toFixed( 1 ) + 'Mbit/s';
				var strFPS = '';
				...
				switch ( rgVideoAdaptations[ r ].roles[ 0 ].toLowerCase() )
				{
					...
					case 'alternate':
						if ( rgVideoAdaptations[ r ].description == rgVideoAdaptations[ r ].language )
							display = '{0}（交替）'.format( CVTTCaptionLoader.LanguageCountryCodes[ rgVideoAdaptations[ r ].language ].displayName );
						else
							display = rgVideoAdaptations[ r ].description;
						break;

					case 'supplementary': // aspect ratio
						display = '{0} 纵横比'.format( rgVideoAdaptations[r].description );
						break;
				...
				switch ( rgAudioAdaptations[ r ].roles[ 0 ].toLowerCase() )
				{
					...
					case 'alternate':
						if ( rgAudioAdaptations[ r ].description == rgAudioAdaptations[ r ].language )
							display = '{0}（交替）'.format( CVTTCaptionLoader.LanguageCountryCodes[ rgAudioAdaptations[ r ].language ].displayName );
						else
					...
					case 'supplementary':
						if ( rgAudioAdaptations[ r ].description == rgAudioAdaptations[ r ].language )
							display = '{0}（补充）'.format( CVTTCaptionLoader.LanguageCountryCodes[ rgAudioAdaptations[ r ].language ].displayName );
						else
							display = rgAudioAdaptations[ r ].description;
						break;

					case 'commentary':
						display = '{0} 解说'.format( CDASHPlayerUI.LocalizeCommentaryDescription( rgAudioAdaptations[ r ].description ) );
						break;
	...
	for (var r = 0; r < rgRepresentation.length; r++)
	{
		...
		switch ( rgRepresentation[r].audioChannels )
		{
			...
			case 2:
				...
				break;
			case 6:
				strChannelInfo = '5.1 环绕';
				break;
			default:
				strChannelInfo = rgRepresentation[r].audioChannels + '-频道';
				break;
		}

		$J('#representation_select_audio').append('<option value="' + ( r ) + '">' + strChannelInfo + ' (' + Math.ceil( rgRepresentation[r].bandwidth / 1000 ) + 'Kbit/s)</option>');
	}
...
CDASHPlayerUI.prototype.SetupVideoRepresentationsInUI = function()
{
	...
	// show "auto" only if there is more than one representation
	if ( rgRepresentation.length > 1 )
		$J( '#representation_select_video' ).append( '<option selected value="-1">自动</option>' );
...
CDASHPlayerUI.prototype.SetupAudioRepresentationsInUITenFoot = function()
{
	...
	if ( rgRepresentation.length > 1 )
	{
		...
		for (var r = 0; r < rgRepresentation.length; r++)
		{
			...
			if ( r == 0 )
				$J( '#representation_audio #representation_select' ).append('<div data-value="' + ( r ) + '" class="selected">' + strChannelInfo + ' (' + Math.ceil( rgRepresentation[r].bandwidth / 1000 ) + 'Kbit/s)</div>');
			else
				$J( '#representation_audio #representation_select' ).append('<div data-value="' + ( r ) + '" class="notselected">' + strChannelInfo + ' (' + Math.ceil( rgRepresentation[r].bandwidth / 1000 ) + 'Kbit/s)</div>');
		}
...
CDASHPlayerUI.prototype.SetupVideoRepresentationsInUITenFoot = function()
{
	...
	if ( rgRepresentation.length > 1 )
	{
		...
		$J( '#representation_video #representation_select' ).append('<div data-value="-1" class="selected">自动</div>');
		...
		for (var r = 0; r < rgRepresentation.length; r++)
		{
			...
			{
				var strResolution = rgRepresentation[r].height + 'p';
				$J( '#representation_video #representation_select' ).append('<div data-value="' + r + '" class="notselected">' + strResolution + ' (' + ( rgRepresentation[r].bandwidth / 1000000 ).toFixed(1) + 'Mbit/s)</div>');
			}
		}
...
CDASHPlayerUI.prototype.OnTimeUpdatePlayer = function()
{
	...
	if ( !this.BInTenFoot() )
	{
		...
		if ( repVideo.find( 'option' ).length > 1 && !repVideo.is( ':focus' ) )
		{
			...
			{
				$J( '#representation_select_video option:first' ).text( '自动 (' + this.m_player.m_nPlaybackHeight + 'p)' );
			}
			else
			{
				$J( '#representation_select_video option:first' ).text( '自动' );
			}
		}
	}
...
CDASHPlayerUI.prototype.InitClosedCaptionOptionPanel = function()
{
	...
	{
		this.m_elClosedCaptionsPanel.append('<div class="cc_style_instructions"><p>Safari 浏览器用户：</p><br/>\n                         <ol>\n                           <li>从 Apple 菜单中选择“系统偏好设置”</li>\n                           <li>点击“辅助功能”</li>\n                           <li>点击“字幕”</li>\n                           <li>选择一个预定义的样式，或者点击 + 创建自定义样式</li>\n                         </ol></div>');
	}
	else if ( this.m_bIsInternetExplorer )
	{
		this.m_elClosedCaptionsPanel.append('<div class="cc_style_instructions"><p>Internet Explorer 浏览器用户：</p><br/>\n                            <ol>\n                              <li>从“工具”菜单中选择“Internet 选项”</li>\n                              <li>点击“辅助功能”</li>\n                              <li>点击“字幕”</li>\n                              <li>开启“忽略默认字幕字体和颜色”</li>\n                              <li>根据您的需要自定义字体、字体样式和颜色</li>\n                              <li>点击“确定”，“确定”，“确定”</li>\n                              <li>重新启动电影以使更改生效</li>\n                            </ol></div>');
	}
	else
	{
