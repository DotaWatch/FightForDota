...
CBroadcastChat.prototype.RequestLoop = function()
{
	...
	.done( function( rgResponse )
	{
		...
		if ( rgResponse.joined )
		{
			for ( var i = 0; i < rgResponse.joined.length; i++ )
			{
				if( rgResponse.joined[i].steamid != _chat.m_steamID )
					_chat.DisplayChatNotification( '%s 加入了聊天'.replace( /%s/, rgResponse.joined[i].persona_name ) );
			}
		}
		...
		if ( rgResponse.left )
		{
			for ( var i = 0; i < rgResponse.left.length; i++ )
			{
				if( rgResponse.left[i].steamid != _chat.m_steamID )
					_chat.DisplayChatNotification( '%s 离开了聊天'.replace( /%s/, rgResponse.left[i].persona_name ) );
			}
		}
		...
		if ( rgResponse.muted )
		{
			...
			{
				var strMsg = ( rgResponse.muted[i].muted == _chat.m_steamID ) ? '您已被禁言，无法发送消息到此聊天' : '%s 在此聊天室中已被禁言';
		...
		{
			_chat.DisplayChatNotification( '已重新连接至聊天' );
...
CBroadcastChat.prototype.AddEmoticons = function( strHTML, steamID, bLocal )
{
	...
	return strHTML.replace( regexEmoticons, '<img class="emoticon" src="http://steamcommunity-a.akamaihd.net/economy/emoticon/$1">' );
};
...
CBroadcastChat.prototype.ChatSubmit = function()
{
	...
	.done( function( response )
	{
		...
		if ( response.result && response.result != 1 )
		{
			...
			if ( response.result == 17 )
				strError = '您已被禁言，无法发送消息到此聊天';
			else
				strError = '发送聊天消息失败：%s'.replace( /%s/, strMessage );
	...
	.fail( function()
	{
		_chat.DisplayChatError( '发送聊天消息失败：%s'.replace( /%s/, strMessage ) );
	});
};
...
CBroadcastChat.prototype.MuteUser = function( steamID, strPersonaName )
{
	...
	if ( !this.m_mapMutedUsers[ steamID ] )
	{
		...
		.fail( function()
		{
						if ( bOwner )
			{
				_chat.DisplayChatError( '禁言 %s 失败。请重试。'.replace( /%s/, strPersonaName ) );
		...
		if ( !bOwner )
		this.DisplayChatNotification( '%s 已被禁言'.replace( /%s/, strPersonaName ) );
};
...
CBroadcastChat.prototype.UnmuteUser = function( steamID, strPersonaName )
{
	...
	if ( bOwner )
	{
		...
		.done( function( response )
		{
			_chat.DisplayChatNotification( '%s 已被解除禁言'.replace( /%s/, strPersonaName ) );
		})
		.fail( function()
		{
			_chat.DisplayChatError( '解除禁言 %s 失败。请重试。'.replace( /%s/, strPersonaName ) );
		});
	}
	else
	{
		this.DisplayChatNotification( '%s 已被解除禁言'.replace( /%s/, strPersonaName ) );
	}
};
...
CBroadcastChat.prototype.OnRemoveUserMessages = function()
{
	...
	.fail( function()
	{
		_chat.DisplayChatError( '删除 %s 的消息失败。请重试。'.replace( /%s/, strPersonaName ) );
	});
